...
vec3 irradianceConvolute(int face, vec2 texCoord)
{
  ...
  for(float phi = 0.0; phi < 2.0 * MATH_PI; phi += irradiance_sampleDelta)
  {
      for(float theta = 0.0; theta < 0.5 * MATH_PI;theta += irradiance_sampleDelta)
      {
...
vec3 prefilterConvolute(int face, vec2 texCoord)
{
	...
	for (uint i = 0u; i < prefilter_SAMPLE_COUNT; ++i)
	{
		...
		float saTexel  = 4.0 * MATH_PI / (6.0 * prefilter_RESOLUTION * prefilter_RESOLUTION);
